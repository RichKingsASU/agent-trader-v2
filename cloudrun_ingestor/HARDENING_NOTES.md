...
-   **The Change:** A signal handler was added to catch `SIGTERM` and `SIGINT`. This handler sets a global `SHUTDOWN_FLAG`. The main `while` loop was modified to check this flag, allowing it to exit cleanly. The loop's `time.sleep()` was replaced with `threading.Event().wait(timeout=...)` so that a shutdown signal can interrupt the sleep period and allow for a fast exit.
...
-   **The Change:** `gunicorn` was added as a dependency. The `Dockerfile` `CMD` was changed to launch the application via `gunicorn`. This provides a stable, battle-tested process manager to run the service indefinitely. We use a single worker (`--workers 1`) and a single thread (`--threads 1`) because this is a single-threaded background job, not a web server handling concurrent user requests. `--timeout 0` is set to prevent Gunicorn from killing the worker during the long `sleep` intervals between publish loops.
