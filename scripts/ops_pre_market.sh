...
# AgentTrader v2 ops snapshot (PRE-MARKET)
...
RUN_DIR="${OUT_ROOT}/ops_runs/${TS_UTC}_pre_market"
...
{
  echo "agenttrader_v2_ops_snapshot: pre_market"
...
if command -v kubectl >/dev/null 2>&1; then
  ...
  # Best-effort metrics (may require metrics-server)
  kubectl -n "${NS}" top pods -l "${LABEL_SELECTOR}" > "${RUN_DIR}/k8s.top_pods.txt" 2>&1 || true

  # Tail logs for critical workloads (best-effort; do not fail snapshot).
