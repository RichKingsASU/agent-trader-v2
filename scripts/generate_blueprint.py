...
{os.linesep.join(logical_lines)}

## Component Inventory (table)

| component name | kind (deploy/sts/job) | image | AGENT_ROLE / AGENT_MODE default | health endpoints (if found) |
|---|---|---|---|---|
{os.linesep.join(comp_rows) if comp_rows else '| unknown | unknown | unknown | unknown | unknown |'}

### Discovered Services (topology context)

{os.linesep.join(svc_lines)}

## Build Pipelines (table)

| cloudbuild file | image output |
|---|---|
{os.linesep.join(pipe_rows) if pipe_rows else '| unknown | unknown |'}

## Safety Controls

{os.linesep.join(safety_lines)}
