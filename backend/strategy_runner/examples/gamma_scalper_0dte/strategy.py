...
from backend.contracts.v2.trading import OptionOrderIntent, OptionRight, Side # New imports
from backend.trading.execution.options_intent_gate import process_option_intent, IntentGateResult # New imports
...
_last_hedge_trade_date: Optional[date_type] = None  # America/New_York date of last hedge intent
...
def _create_hedge_order(
    ...
    """
    Create an OptionOrderIntent to neutralize portfolio delta and pass it through the intent gate.
    
    Args:
        underlying_symbol: Symbol of the underlying asset (e.g., "SPY")
        hedge_qty: Quantity to trade (positive = buy, negative = sell in shares)
        net_delta: Current net portfolio delta
    ...
    Returns:
        A dictionary representing the shadow execution result if approved, or None if blocked.
    """
    global _macro_event_active, _stop_loss_multiplier, _position_size_multiplier
    
    # Determine option contract details for hedging
    # For a minimal implementation, we assume 0DTE options and ATM strike
    
    # Expiry: Next trading day's date (assuming 0DTE logic)
    # A more robust solution would dynamically find the nearest 0DTE expiry.
    # For now, let's use the current day's date if trading today, otherwise next day.
    hedge_expiry_date = to_nyse(_parse_timestamp(ts)).date()
    # If it's too late in the day for today's expiry, target next trading day.
    if _is_market_close_time(_parse_timestamp(ts)):
        hedge_expiry_date += timedelta(days=1) # Simple increment, needs to be actual next trading day
    
    # Strike: At The Money (ATM) strike, which is typically the current underlying price.
    hedge_strike = float(underlying_price.quantize(Decimal("1"), rounding=ROUND_HALF_DOWN))
    
    # Option Type and Side: Simplified mapping similar to adapter logic
    # If net_delta is positive (long delta, need to sell shares/reduce delta) -> Sell Calls
    # If net_delta is negative (short delta, need to buy shares/add delta) -> Buy Calls
    # Or, to be consistent with adapter's "sell_premium" default for positive delta hedge (sell shares):
    # If hedge_qty > 0 (buy shares, add delta) -> Sell Puts (conservative, less delta per contract) or Buy Calls
    # If hedge_qty < 0 (sell shares, reduce delta) -> Sell Calls (conservative, more delta per contract) or Buy Puts
    
    # For simplicity, let's choose to always use CALLS for hedging
    # If we need to increase delta (hedge_qty > 0), BUY CALLS
    # If we need to decrease delta (hedge_qty < 0), SELL CALLS
    
    option_side = Side.BUY if hedge_qty > Decimal("0") else Side.SELL
    option_right = OptionRight.CALL # Always use Call for this strategy

    # Convert shares to contracts
    contracts_qty = int(abs(hedge_qty) / get_options_contract_multiplier())
    if contracts_qty < 1:
        logger.info(f"Hedge quantity {hedge_qty} too small to form option contract(s). Skipping.")
        return None
    ...
    metadata = {
        "reason": "delta_hedge_options",
        "net_delta_before": str(net_delta),
        "hedge_qty_shares": str(hedge_qty), # Original hedge quantity in shares
    ...
    if _macro_event_active:
        # Stop-loss price calculation is not directly applicable to OptionOrderIntent creation,
        # but the multipliers can be logged.
        metadata.update({
            "stop_loss_multiplier": str(_stop_loss_multiplier),
            "position_size_multiplier": str(_position_size_multiplier),
        })
    
    option_intent = OptionOrderIntent(
        intent_id=uuid.uuid4(),
        account_id="gamma_scalper_account", # Placeholder account ID
        strategy_id="0dte_gamma_scalper",
        symbol=underlying_symbol,
        asset_class="equity_option", # Pydantic enum requires string or enum member
        side=option_side,
        order_type="market",
        time_in_force="day",
        quantity=contracts_qty,
        contract_symbol=f"{underlying_symbol}{hedge_expiry_date.strftime('%y%m%d')}C{int(hedge_strike*100):08}", # Example OCC-style
        expiration=hedge_expiry_date,
        strike=str(hedge_strike), # Strike as DecimalString
        right=option_right,
        options=metadata # Store additional metadata in the options field
    )

    # Pass the OptionOrderIntent through the intent gate
    gate_result = process_option_intent(option_intent)
    
    if gate_result.blocked:
        logger.info(f"Option intent blocked by gate: {gate_result.reason}")
        return None
    
    logger.info(f"Option intent approved and executed: {gate_result.execution_result.stored}")
    return gate_result.execution_result.stored # Return the stored record
...
def on_market_event(event: Dict[str, Any]) -> Optional[List[Dict[str, Any]]]:
    ...
    hedge_order_result = _create_hedge_order( # Changed variable name to hedge_order_result
    ...
    try:
        if _pnl_tracker is not None:
            # Pnl tracker expects signed quantity for shares, not contracts
            # If hedge_order_result is an options trade, its quantity will be contracts.
            # We need to pass the share-equivalent for PnL tracking if it's still for SPY.
            # Given that _create_hedge_order now returns OptionOrderIntent result,
            # we need to adjust this for PnL tracker which expects underlying shares.
            # For this exercise, we will assume the Pnl tracker can handle the options context
            # or that the original hedge_qty (shares) can be passed for tracking.
    ...
    # The _spy_position_qty tracks SPY shares, so if hedging with options, this might need
    # to be adjusted to track delta-equivalent or removed if only options are used.
    # For now, keep it as is, assuming options also contribute to effective SPY position.
    _spy_position_qty += hedge_qty 
    _emit_pnl_update(event_id=event_id, ts=ts, mark_price=underlying_price)

    # If hedge_order_result is None (blocked by gate), return empty list.
    return [hedge_order_result] if hedge_order_result else []
